...
/**
 * GitHub Deep Deployment Cleanup Script
 * Aggressively deletes old deployments by:
 * 1. Deactivating all deployment statuses
 * 2. Deleting deployment records
 * 3. Removing environments
 * 
 * Usage: node scripts/cleanup-github-deep.js <github-token> <owner> <repo> [--keep-count N] [--dry-run]
 */
...
async function listDeploymentStatuses(token, owner, repo, deploymentId) {
  const url = `https://api.github.com/repos/${owner}/${repo}/deployments/${deploymentId}/statuses?per_page=100`;
...
async function createDeploymentStatus(token, owner, repo, deploymentId, state) {
  const url = `https://api.github.com/repos/${owner}/${repo}/deployments/${deploymentId}/statuses`;
  
  const body = {
    state: state,
    description: 'Cleanup script - marking as inactive',
    environment: 'cleanup',
    auto_inactive: true,
  };
...
async function main() {
  ...
  if (nonFlagArgs.length < 3) {
    console.error('Usage: node scripts/cleanup-github-deep.js <token> <owner> <repo> [--keep-count N] [--dry-run]');
    console.error('Example: node scripts/cleanup-github-deep.js gh_abc123 danniel0976 twylm --keep-count 5 --dry-run');
    process.exit(1);
  }
  ...
  console.log('    Strategy: Deactivate statuses first, then delete');
  ...
  console.log('\n🗑️  Step 1: Deactivating deployment statuses...\n');
  
  let deactivateSuccess = 0;
  let deactivateFailed = 0;
  
  for (const deployment of toDelete) {
    process.stdout.write(`  Deactivating ${deployment.sha.substring(0, 7)}... `);
    try {
      await createDeploymentStatus(token, owner, repo, deployment.id, 'inactive');
      console.log('✅');
      deactivateSuccess++;
    } catch (err) {
  ...
  console.log(`\n✨ Deactivation: ${deactivateSuccess} done, ${deactivateFailed} failed`);
  
  console.log('\n🗑️  Step 2: Deleting deployments...\n');
  ...
  if (environments.length > 0) {
    console.log('\n🗑️  Step 3: Deleting environments...\n');
